...
class Transmitter extends ChannelInboundHandlerAdapter {
  ...
  override def channelActive(ctx: ChannelHandlerContext): Unit = {
    ...
    // Write the message into a byte buffer
    val message = (System.currentTimeMillis() / 1000L + 2208988800L).toInt
    val byteBuffer = ctx.alloc().buffer(4)
    byteBuffer.writeInt(message)
    ...
    channelFuture.addListener(new ChannelFutureListener() {
      ...
      override def operationComplete(future: ChannelFuture): Unit = {
        ...
        println(s"""Transmitter listener \"operationComplete\" is invoked...: if the argument \"future\" is the same as \"channelFuture\"? ${future == channelFuture}""")
        ctx.close()
      }
    })
  }

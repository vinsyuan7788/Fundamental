package org.lang.scala.conclusion.dandc.concurrent.java.structure.blocking.queue
...
/**
 * 	This is a stand-alone object to test LinkedBlockingQueue class
 *  -- If there is no element inside LinkedBlockingQueue, this LinkedBlockingQueue can be blocked until there is new element coming in
 *     -- E.g., when invoking "take", "put" method, etc.
 *  -- LinkedBlockingQueue will take a lock before any modification (not using CAS)
 *  -- Suitable for situation where blocking is needed
 * 
 * 	@author VinceYuan
 */
object TestLinkedBlockingQueue {
  ...
  /**
   * 	This is a method to test LinkedBlockingQueue class
   */
  private def testLinkedBlockingQueue(): Unit = {
